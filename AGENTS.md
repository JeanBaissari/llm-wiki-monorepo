...
- **Python (v0.2.0+):** openai, anthropic, litellm, instructor, tenacity, pydantic, portalocker — specified in `pyproject.toml`. See [Python Dependency Policy](#python-dependency-policy-v020).
- **Optional Python extras (legacy, opt-in, NOT the default story):** `[semantic]`, `[eval]`, `[entity-resolution]`, `[ner]`, `[leiden]`, `[recommended]`. The project is agent-native — intelligence comes from the external agent; the base install stays lexical-only with pure-Python fallbacks and performs no model downloads. Do not promote these in docs or add new CI lanes that download models.
- No Rust dependencies.

### Local-model surfaces (policy)

Local-model machinery is out of the default story by owner decision (2026-09): no Ollama, no HuggingFace/model downloads, no torch/ONNX weights, no docs/tooling promoting them. The `[ner]` extra (gliner/onnxruntime) is retained only as a legacy opt-in and degrades to the regex extractor byte-identically when absent; its CI lane is manual/nightly, never on push/PR. The graphify (`@sentropic/graphify`) JS surface was fully removed in v0.6.4 (ADR-0035).
