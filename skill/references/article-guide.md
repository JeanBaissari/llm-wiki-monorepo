...
<What the comparison reveals. Which approach wins on which dimensions.>

## Sources

- [[summaries/source-slug-1]]
- [[summaries/source-slug-2]]
```
...
On pages that synthesize claims from 3+ sources, append provenance markers at the end of paragraphs to trace claims back to specific sources:

```markdown
The model achieves 94.3% on the benchmark, outperforming prior approaches. ^[raw/articles/source-slug.md]
```

This lets a reader verify each claim without re-reading every raw file. Optional on single-source pages where the `sources:` frontmatter is enough.
