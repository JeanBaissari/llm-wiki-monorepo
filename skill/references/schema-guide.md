...
This wiki follows the llm-wiki skill's eight operations: `compile`, `ingest`, `ingest-2step`, `query`, `lint`, `audit`, `research`, `insights`.
...
- **Comparison pages** (`wiki/comparisons/`): Side-by-side analyses of two or more entities or concepts with dimensions, sources, and verdict.
...
- `confidence: high | medium | low` — how well-supported the claims are. Use `medium` or `low` for single-source claims; reserve `high` for well-supported claims across multiple sources.
- `contested: true` — set when the page has unresolved contradictions with other pages.
- `contradictions: [other-page-slug, ...]` — list of pages this one conflicts with.
- `entity_type: person | tool | paper | organization` — additional field for entity pages.
...
Files in `raw/` also get frontmatter for drift detection:
```yaml
---
...
---
```
The `sha256:` field lets the lint script detect when raw files have been modified or when a re-ingested URL has changed content.
...
- **Do NOT create a page** for passing mentions, minor details, or things outside the wiki's scope.
