...
def _call_llm_impl(system: str, user: str, provider: str = "default") -> Optional[str]:
    """Inner implementation: subprocess-based LLM dispatch (no timeout wrapping)."""
...
def call_llm(system: str, user: str, provider: str = "default",
             total_timeout: Optional[int] = None) -> Optional[str]:
    """Call the LLM with an optional total timeout spanning all retries.

    When total_timeout is set, the call is wrapped in a ThreadPoolExecutor
    so that the entire operation (including subprocess retries) is aborted
    if it exceeds the deadline. This is a budget/cost control boundary —
    SDK-level timeouts handle individual network issues.

    Args:
        system: System prompt.
        user: User prompt.
        provider: LLM provider name.
        total_timeout: Total deadline in seconds. Default: None (no limit).

    Returns:
        The model's text response, or None on failure/timeout.
    """
    if total_timeout is not None:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(_call_llm_impl, system, user, provider)
            try:
    ...
    return _call_llm_impl(system, user, provider)
...
def write_wiki(root: str, rpath: str, content: str, pages_dir: str = None,
               force: bool = False, lock_timeout: Optional[int] = None) -> tuple:
    if pages_dir:
        parts = rpath.split("/", 1)
        if len(parts) == 2:
            rpath = parts[1]
    ...
    try:
        ...
        from content_hash import read_hash, inject_hash
        ...
        # Fallback to original stdlib behavior
