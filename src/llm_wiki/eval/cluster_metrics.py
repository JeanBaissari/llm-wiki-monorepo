"""
src/llm_wiki/eval/cluster_metrics.py

Community-detection verification metrics — canonical implementation:
  - NMI (Normalized Mutual Information) — sklearn variant with pure-Python fallback
  - ARI (Adjusted Rand Index)
  - Modularity Q

When sklearn is available, uses sklearn.metrics for NMI/ARI. Falls back to
pure-Python implementations if sklearn is not installed. Moved verbatim from
``tests/verification/metrics.py`` so the production gate (ADR-0027 §gate) and
the verification suite share one implementation (tests/verification/metrics.py
