...
def test_two_signal_rule_requires_both():
    ...
    # neural network / networks: high string AND high embedding → merge.
    m1 = merges_to_clusters(resolve_entities(["Neural Network", "Neural Networks"], embedder=emb))
    assert any({"Neural Network", "Neural Networks"} <= c for c in m1)

    # K8s / Kubernetes: high embedding but LOW string → must stay unmerged
    # (embedding alone can never justify a merge — ADR-0021/0024).
    m2 = merges_to_clusters(resolve_entities(["Kubernetes", "K8s"], embedder=emb))
    assert m2 == []
